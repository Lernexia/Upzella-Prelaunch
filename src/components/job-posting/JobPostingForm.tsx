...
// Predefined CRM providers with generated API keys and endpoints
const initialCRMConfig: CRMIntegrationData = {
  providers: [
    { id: uuidv4(), name: 'Salesforce', enabled: false, apiKey: generateAPIKey(), endpoint: generateEndpoint('Salesforce') },
    { id: uuidv4(), name: 'HubSpot', enabled: false, apiKey: generateAPIKey(), endpoint: generateEndpoint('HubSpot') },
    { id: uuidv4(), name: 'Zoho Recruit', enabled: false, apiKey: generateAPIKey(), endpoint: generateEndpoint('Zoho Recruit') },
  ]
}
...
const JobPostingForm: React.FC<{
  CJobDetails?: JobDetailsData,
  ResumeConfig?: ResumeParsingData,
  CRMConfig?: CRMIntegrationData,
  isEditing?: boolean,
  prevJobUrl?: string
}> = ({
  CJobDetails, ResumeConfig, CRMConfig, isEditing = false, prevJobUrl = ''
}) => {

    const jobUuid = crypto.randomUUID();
    const jobUrl = prevJobUrl ? prevJobUrl : `https://demo-app.upzella.in/demo/jobs/${jobUuid}`;
    ...
    // Form data states
    const [jobDetails, setJobDetails] = useState<JobDetailsData>(CJobDetails || initialJobDetails);
    const [resumeConfig, setResumeConfig] = useState<ResumeParsingData>(ResumeConfig || initialResumeConfig);
    const [crmConfig, setCrmConfig] = useState<CRMIntegrationData>(CRMConfig || initialCRMConfig);
    ...
    return (
      <div className="max-w-3xl mx-auto pb-12">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-upzella-primary">
            {isEditing ? 'Edit Job Posting' : 'Create Job Posting'}
          </h2>
          <button
            type="button"
            onClick={() => setShowAutofillModal(true)}
            className="inline-flex items-center text-sm font-medium bg-upzella-accent/30 text-upzella-primary px-3 py-1.5 rounded-md hover:bg-upzella-accent/50 transition-colors bg-white"
          >
